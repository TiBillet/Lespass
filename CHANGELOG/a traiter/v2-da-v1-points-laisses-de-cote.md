...
- **Footer** : cibles des liens « Le réseau des lieux », « Journal », « À propos »
  (masqués) ; URL de « Faire un don à TiBillet » (`href="#"`) ; logo TiBillet ou
  lien tibillet.coop sur la marque.
...
- `pages/V2/vues/accueil.html` : l'ancienne carte événement en dur est gardée en
  commentaire (bloc `V2-HERE` des sections masquées) le temps de la recette,
  puis à supprimer. Le bloc `main_old` (jamais rendu) aussi.
- `pages/V2/partials/evenement_accordeon.html` et `pages/V2/partials/carte_evenement.html`
  ne sont plus inclus par les gabarits V2 : à supprimer après vérification
  (`rg "evenement_accordeon|carte_evenement" pages/ BaseBillet/`).
- `pages/V2/vues/compte/punchclock.html` : l'id `badgeOut` est répété dans une
  boucle (existant, conservé tel quel pour ne pas changer d'id).
- Maquette : `test-double-nav-v1.html` (prototype abandonné) et `data.js`
  (jamais chargé) sont obsolètes.
