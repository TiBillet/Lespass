<template>
    <fieldset class="shadow-sm p-3 mb-5 bg-body rounded"
            v-for="(product, index) in event.products.filter(prod => prod.categorie_article === 'S')"
            :key="index">
    <legend>
    ...
    <!-- les prix -->
    <div class="input-group mb-2" v-for="(prix, index2) in product.prices" :key="index2">
      ...
      <input id="charge_cashless" :value="getChargeCashless"
             ...
             :data-uuid="prix.uuid" class="form-control">
    </div>
...
<script setup>
console.log('-> CardCreditCashless_save.vue')
// store
