...
| 24. Relecture Opus des corrections (2026-09-23) | Trouvé et corrigé : `MembershipValidator` lisait le montant avec `attrs.get('custom_amount') or self.price.prix` — un montant saisi à 0 était remplacé en silence par le minimum du tarif (adhésion créée et facturée 10 € au lieu d'un refus). Atteignable par l'API v2 (`api_v2/serializers.py:1571` recopie `customAmount`). Test de parité vu rouge (sans panier ; le panier refusait déjà). Commentaires de la correction précédente réécrits au présent (règle djc) |
| 25. Suite de la relecture Opus (2026-09-23) | Ligne ressource du panier : montant et calcul affichés pour un tarif horaire de 0 € (même piège « 0 est faux »). Estimation d'un créneau arrondie au centime par `dround`, comme le prix vendu — une durée de 40 ou 50 minutes donnait 28 décimales affichées et un centime d'écart avec Stripe. Réservation gratuite en attente d'activation ajoutée aux 4 requêtes de « mes réservations » (elle occupait son créneau sans apparaître nulle part). `get_or_create_price_sold` : `if custom_amount is not None`. `validate_new_booking` : plus d'exception pour un prix manquant, un refus comme les autres. 3 commentaires réécrits au présent. 3 tests neufs vus rouges |
