...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            action="store_true",
            dest="no_input",
            help="Pas de confirmation interactive (pour usage en script).",
        )
    ...
    def _handle_quick(self, options):
        ...
        gunzip = subprocess.Popen(
            ["gunzip", "-c", dump_path], stdout=subprocess.PIPE, env=env
        )
        ...
        if psql.returncode != 0:
            self.stderr.write(
                self.style.ERROR(
                    f"Erreur lors de la restauration (code {psql.returncode})."
                )
            )
            return
    ...
    def _dump_database(self):
        ...
        try:
            ...
            if pg_dump.wait() != 0 or gzip_proc.returncode != 0:
                self.stderr.write(
                    self.style.ERROR("Erreur lors de l'export du dump SQL.")
                )
                return
    ...
    def _handle_full(self, options):
        ...
        fixtures = [
            {
                "name": "Lespass",
                "short_description": "Instance de démonstration  du collectif imaginaire «Lespass».",
                "long_description": (
                ...
                "stripe_connect_account_test": os.environ.get(
                        ...
                        "products": [
                            {
                                ...
                                "form_fields": [
                                    ...
                                    },
                                    {
                                        ...
                                        "required": False,
                                        "order": 3,
                                        "help_text": "Indiquez un numéro pour vous joindre le jour J.",
                                    },
                                    {
                                        "label": "Type de lieu",
                                    ...
                                    },
                                    {
                                        ...
                                        "required": False,
                                        "order": 7,
                                        "help_text": "Dites-nous en quelques phrases ce qui vous motive.",
                                    },
                                    {
                        ...
                        "prices": [
                            {
                                ...
                                "recurring_payment": True,
                                "subscription_type": "MONTH",
                                "short_description": "Panier DUO chaque mois — engagement 6 mois",
                                "iteration": 6,
                                "commitment": True,
                                "stock": 30,
                            },
                            {
                                ...
                                "recurring_payment": True,
                                "subscription_type": "MONTH",
                                "short_description": "Panier chaque mois — engagement 6 mois",
                                "iteration": 6,
                                "commitment": True,
                                "stock": 20,
                            },
                            {
                                ...
                                "prix": 400,
                                "subscription_type": "YEAR",
                                "short_description": "Engagement annuel",
                                "stock": 50,
                            },
                        ],
                    },
                    {  # Adhésion à validation manuelle (non sélective)
                        ...
                        "form_fields": [
                            # FALC: Formulaire d'adhésion pour une coopérative (tous les types inclus)
                            {
                                ...
                                "required": True,
                                "order": 1,
                                "help_text": "Indiquez votre nom et prénom, ou la dénomination de la structure.",
                            },
                            {
                                ...
                                "required": True,
                                "order": 2,
                                "help_text": "Rue, code postal et ville.",
                            },
                            {
                            ...
                            {
                                "label": "Domaines d'intérêt dans la coopérative",
                                "field_type": "MULTI_SELECT",
                                "required": False,
                                "order": 4,
                                "options": [
                            ...
                            {
                                "label": "Préférence de convocation aux AG",
                                "field_type": "RADIO_SELECT",
                                "required": True,
                                "order": 5,
                                "options": ["Email", "Courrier postal"],
                            },
                            {
                                "label": "J'atteste avoir lu et accepte les statuts",
                                "field_type": "BOOLEAN",
                                "required": True,
                                "order": 6,
                                "help_text": "Case obligatoire pour poursuivre.",
                            },
                            {
                                "label": "J'accepte de figurer sur le registre des sociétaires",
                                "field_type": "BOOLEAN",
                                "required": False,
                                "order": 7,
                            },
                            {
                                "label": "Numéro de pièce d'identité (optionnel)",
                                "field_type": "SHORT_TEXT",
                                "required": False,
                                "order": 8,
                            },
                            {
                                "label": "Motivations et projet au sein de la coopérative",
                                "field_type": "LONG_TEXT",
                                "required": False,
                                "order": 9,
                            },
                        ...
                        "prices": [
                            {
                                "name": "Souscription structure morale",
                                "prix": 500,
                                "subscription_type": "LIFE",
                                "manual_validation": True,
                                "short_description": "Validation requise",
                            },
                            {
                                "name": "Souscription particulier",
                                "prix": 50,
                                "subscription_type": "LIFE",
                                "manual_validation": True,
                                "short_description": "Validation requise",
                ...
                "initiatives": [
                    # FALC: Les initiatives ci‑dessous couvrent tous les cas d’usage (LES PASS uniquement)
                    {
                        ...
                        "budget_items": [
                            {
                                "contributor": "douglas.adams@h2g2.org",
                                "description": "Frais d'auto-stop",
                                "amount": 15,
                            },
                            {
                                "contributor": "thor@dirkgently.org",
                                "description": "Scénario",
                                "amount": 800,
                            },
                            {
                                "contributor": "fondation@h2g2.org",
                                "description": "Script",
                                "amount": 600,
                            },
                            {
                    ...
                    },
                    {
                        ...
                        "budget_items": [
                            {
                                "contributor": "zaphod@h2g2.org",
                                "description": "Aménagement du garage pour le coeur en or",
                                "amount": 10000,
                            },
                            {
                                "contributor": "douglas.adams@h2g2.org",
                        ...
                        "participations": [
                            {
                                ...
                                "requested_value": 100,
                                "description": "Frais de pressing pour robe de chambre",
                                "state": "APPROVED_ADMIN",
                            },
                            {
                                "user_email": "zaphod@h2g2.org",
                                "requested_value": 1000,
                                "description": "Décoration : Portrait du président de la galaxie",
                                "state": "REQUESTED",
                            },
                            {
                                "user_email": "souris@h2g2.org",
                                "requested_value": 4200,
                                "description": "Construction de Compute-1",
                                "state": "APPROVED_ADMIN",
                            },
                            {
                                "user_email": "dauphins@h2g2.org",
                                "requested_value": 420,
                                "description": "Salut, et merci pour le poisson !",
                                "state": "APPROVED_ADMIN",
                            },
                            {
                                "user_email": "ford@h2g2.org",
                                "requested_value": 200,
                                "description": "Impression du guide sur papier",
                                "state": "REQUESTED",
                            },
                        ],
                    },
                    {
                        ...
                        "budget_items": [
                            {
                                "contributor": "arthur.dent@h2g2.org",
                                "description": "La méthode : Louper le sol en une trilogie de 5 volumes",
                                "amount": 10000,
                            },
                        ],
                        "votes": [
            ...
            },
            {
                ...
                "events": [
                    ...
                    {
                        "name": "Atelier couture — prix libre",
                        "categorie": "ATELIER",
                        "reservation": "payante",
                        "products": [
                            {
                ...
                "initiatives": [
                    {
                        "name": "Réparer le fournil collectif",
                        "budget_contributif": True,
                        "currency": "€",
                    },
                ],
            },
            {
                ...
                "events": [
                    {
                        "name": "Scène ouverte acoustique (entrée libre)",
                        "categorie": "CONCERT",
                        "reservation": "aucune",
                        "tags": ["Gratuit", "Entrée libre", "Jazz"],
                    },
                    {
                        "name": "Conférence écologie populaire",
                        "categorie": "CONFERENCE",
                ...
                "initiatives": [
                    {
                        "name": "Fresque murale participative",
                        "budget_contributif": False,
                        "currency": "€",
                    },
                ],
            ...
            },
            {
                ...
                "events": [
                    ...
                    {
                        "name": "Fête des voisins — prix libre",
                        "categorie": "FETE",
                        "reservation": "payante",
                        "products": [
                            {
                ...
                "adhesions": [
                    {
                        "name": "Adhésion Maison des Communs",
                        "categorie_article": "ADHESION",
                        "prices": [
                            {
                                "name": "Mensuelle",
                ...
                "initiatives": [
                    {
                        "name": "Outils partagés — achat collectif",
                        "budget_contributif": True,
                        "currency": "€",
                    },
                    {
                        "name": "Documentation des savoir-faire",
                        "budget_contributif": False,
                        "currency": "€",
                    },
                ],
            ...
            },
            {
                ...
                "adhesions": [
                    {
                        "name": "Adhésion réseau — Individuel",
                        "categorie_article": "ADHESION",
                        "prices": [
                            {
                                "name": "Annuelle",
                    ...
                    {
                        "name": "Adhésion réseau — Structure",
                        "categorie_article": "ADHESION",
                        "prices": [
                            {
                                "name": "Annuelle",
        ...
        # -----------------------------
        # 0) Option --flush: purge complete des donnees de demo des tenants cibles
        # Ordre de suppression : enfants d'abord (FK), parents ensuite.
        # Preserve : tenants, config, appairage Fedow, users admin, wallets du lieu.
        # Supprime : tout le reste (produits, events, laboutik, fedow_core, comptabilite, wallets users, cartes NFC).
        # / Complete flush of demo data for target tenants.
        # Deletion order: children first (FK), parents last.
        # Preserved: tenants, config, Fedow handshake, admin users, venue wallets.
        # Deleted: everything else (products, events, laboutik, fedow_core, accounting, user wallets, NFC cards).
        # -----------------------------
        if options.get("flush"):
            # Si --no-input, pas de confirmation interactive (usage en script / flush.sh)
            # / If --no-input, skip interactive confirmation (script usage / flush.sh)
            no_input = options.get("no_input", False)
            if not no_input:
                try:
                    confirmation = input(
                        "PURGER toutes les donnees de demo (produits, events, laboutik, fedow_core, comptabilite) "
                        "des tenants de demo ? Tapez 'oui' pour confirmer: "
                    )
                except Exception:
                    confirmation = ""
                if confirmation.strip().lower() not in ["oui", "yes"]:
                    self.stdout.write(
                        self.style.WARNING(
                            "Opération annulée: aucune donnée supprimée. Relancez avec --flush et confirmez par 'oui'."
                        )
                    )
                    return
            ...
            laboutik_installed = "laboutik" in settings.INSTALLED_APPS
            inventaire_installed = "inventaire" in settings.INSTALLED_APPS
            fedow_core_installed = "fedow_core" in settings.INSTALLED_APPS
            ...
            with schema_context("public"):
                ...
                # / Order: Transaction -> Token -> Asset -> Federation
                if fedow_core_installed:
                    try:
                        tx_deleted = Transaction.objects.count()
                        if tx_deleted:
                            Transaction.objects.all().delete()
                        tk_deleted = Token.objects.count()
                        if tk_deleted:
                            Token.objects.all().delete()
                        asset_deleted = FedowAsset.objects.count()
                        if asset_deleted:
                            FedowAsset.objects.all().delete()
                        fed_v2_deleted = Federation.objects.count()
                        if fed_v2_deleted:
                            Federation.objects.all().delete()
                        logger.info(
                            f"--flush fedow_core (public): transactions={tx_deleted}, "
                            f"tokens={tk_deleted}, assets={asset_deleted}, federations={fed_v2_deleted}"
                        )
                    except Exception as e:
                ...
                try:
                    ...
                    detail_deleted = Detail.objects.count()
                    if detail_deleted:
                        Detail.objects.all().delete()

                    # Supprimer uniquement les wallets users (origin=null)
                    # Les wallets du lieu (origin=tenant) sont preserves
                    # / Delete only user wallets (origin=null)
                    # Venue wallets (origin=tenant) are preserved
                    wallets_users = Wallet.objects.filter(origin__isnull=True)
                    wallet_deleted = wallets_users.count()
                    if wallet_deleted:
                        wallets_users.delete()
                    logger.info(
                        f"--flush (public): wallets_users={wallet_deleted}, "
                        f"cartes_cashless={carte_deleted}, details={detail_deleted}, "
                        f"assets_fedow_connect={fc_asset_deleted}"
                    )
                except Exception as e:
        ...
        with schema_context("public"):
            ...
            for fx in fixtures:
                ...
                try:
                    ...
                    logger.warning(
                        f"Impossible de créer le schéma '{schema}' pour le tenant {name}: {e}"
                    )
                    raise ValueError(
                        f"Impossible de créer le schéma '{schema}' pour le tenant {name}: {e}"
                    )
                ...
                try:
                    ...
                    domain_obj, _ = Domain.objects.get_or_create(
                        domain=domain_str, tenant=tenant, defaults=dict(is_primary=True)
                    )
                    ...
                    logger.warning(
                        f"Impossible d'assurer le domaine principal pour {name}: {e}"
                    )
        ...
        for tenant in created_tenants:
            with tenant_context(tenant):
                ...
                try:
                    admin_user: TibilletUser = get_or_create_user(
                        fx.get("email"), send_mail=False
                    )
                ...
                if addr_data:
                    addr_defaults = {
                        ...
                        "address_region": addr_data.get("address_region")
                        if "address_region" in addr_data
                        else None,
                ...
                try:
                    ...
                    config.stripe_connect_account_test = fx.get(
                        "stripe_connect_account_test"
                    )
                    config.stripe_payouts_enabled = bool(
                        fx.get("stripe_payouts_enabled")
                    )
                    ...
                    logger.warning(
                        f"Impossible de configurer Configuration pour {tenant.name}: {e}"
                    )
                    ...
                    try:
                        ...
                        for a in declared_assets:
                            try:
                                ...
                                obj = AssetFedowPublic.objects.filter(
                                ...
                                logger.warning(
                                    f"Asset Fedow non créé/assuré pour '{tenant.name}': {e}"
                                )
                    except Exception as e:
                        logger.warning(
                            f"Impossible d'initialiser les assets Fedow pour {tenant.name}: {e}"
                        )
                ...
                for adh in fx.get("adhesions", []) or []:
                    prod, _ = Product.objects.get_or_create(
                        name=adh["name"],
                        defaults={
                            "categorie_article": map_product_categorie(
                                adh.get("categorie_article") or "ADHESION"
                            ),
                    ...
                    for ff in adh.get("form_fields", []) or []:
                        try:
                            ProductFormField.objects.get_or_create(
                                ...
                                defaults={
                                    "field_type": map_form_field_type(
                                        ff.get("field_type")
                                    ),
                            ...
                            logger.warning(
                                f"Impossible de créer ProductFormField pour l'adhésion '{prod.name}': {e}"
                            )
                    # Tarifs
                    for pr in adh.get("prices", []) or []:
                        ...
                        try:
                            if pr.get("fedow_reward_enabled"):
                                ...
                                fedow_defaults["fedow_reward_amount"] = pr.get(
                                    "fedow_reward_amount"
                                )
                                ...
                                asset_obj = None
                                if asset_uuid:
                                    asset_obj = AssetFedowPublic.objects.filter(
                                        uuid=asset_uuid
                                    ).first()
                                    if not asset_obj:
                                        logger.warning(
                                            f"Asset UUID '{asset_uuid}' introuvable pour le tenant {tenant.name}. Récompense ignorée."
                                        )
                                if not asset_obj and asset_name:
                                    # Cherche dans la base du tenant, sinon dans les assets déclarés
                                    asset_obj = AssetFedowPublic.objects.filter(
                                        name=asset_name
                                    ).first() or assets_by_name.get(asset_name)
                                if asset_obj:
                                    fedow_defaults["fedow_reward_asset"] = asset_obj
                                else:
                                    logger.info(
                                        f"Aucun asset Fedow lié pour le tarif '{pr.get('name')}' (produit '{prod.name}') — définissez 'fedow_reward_asset' ou 'fedow_reward_asset_name' et déclarez l'asset dans le JSON."
                                    )
                        except Exception as e:
                            logger.warning(
                                f"Impossible de traiter la récompense Fedow pour '{adh.get('name')}': {e}"
                            )
                ...
                for ev in fx.get("events", []) or []:
                    ...
                    event_obj, created_event = Event.objects.get_or_create(
                        ...
                        defaults={
                            "datetime": when,
                            "categorie": map_event_categorie(
                                ev.get("categorie") or "CONCERT"
                            ),
                    ...
                    if (
                        ...
                        try:
                            ...
                            if "monnaie temps".lower() not in new_ld.lower():
                                new_ld = (
                                    ev.get("long_description") or new_ld or ""
                                ) + bonus_text
                    ...
                    for prod_def in ev.get("products", []) or []:
                        ...
                        for pr in prod_def.get("prices", []) or []:
                            defaults = {
                                ...
                                "subscription_type": getattr(
                                    Price,
                                    (pr.get("subscription_type") or "NA").upper(),
                                    Price.NA,
                                ),
                            }
                            ...
                            if adhesion_name and _created:
                                try:
                                    ...
                                    logger.warning(
                                        f"Adhésion requise '{adhesion_name}' introuvable pour '{p.name}'."
                                    )
                        ...
                        for ff in prod_def.get("form_fields", []) or []:
                            try:
                                ...
                                logger.warning(
                                    f"Impossible de créer ProductFormField pour '{p.name}': {e}"
                                )
                    ...
                    for child in ev.get("children", []) or []:
                        ...
                        child_when = when + timedelta(
                            days=random.randint(0, 14), hours=random.randint(1, 6)
                        )
                        ...
                        try:
                            ...
                            price = (
                                Price.objects.filter(product=action_prod)
                                .order_by("order")
                                .first()
                            )
                            if not price:
                            ...
                            if time_asset and price.fedow_reward_asset_id != getattr(
                                time_asset, "uuid", None
                            ):
                            ...
                            logger.warning(
                                f"Impossible de configurer la récompense monnaie temps pour '{child.get('name')}' : {e}\n"
                            )
                    ...
                    for init in inits:
                        ...
                        try:
                            ...
                            initiative_obj, _ = Initiative.objects.get_or_create(
                                name=name,
                                defaults=dict(
                                    short_description=init.get("short_description")
                                    or "",
                                    description=init.get("description") or "",
                                    archived=bool(init.get("archived", False)),
                                    vote=bool(init.get("vote", False)),
                                    budget_contributif=bool(
                                        init.get("budget_contributif", False)
                                    ),
                                    adaptative_funding_goal_on_participation=bool(
                                        init.get(
                            ...
                            for fld, fxkey, transform in [
                                ...
                                val = init.get(fxkey)
                                if val is not None and getattr(
                                    initiative_obj, fld
                                ) != transform(val):
                            ...
                            for bfield in [
                                ...
                                if bfield in init and getattr(
                                    initiative_obj, bfield
                                ) != bool(init.get(bfield)):
                                    setattr(
                                        initiative_obj, bfield, bool(init.get(bfield))
                                    )
                                    fields_to_update.append(bfield)
                            if (
                                "funding_mode" in init
                                and initiative_obj.funding_mode
                                != init.get("funding_mode")
                            ):
                            ...
                            if asset_name:
                                try:
                                    ...
                                    if (
                                        asset_obj
                                        and getattr(initiative_obj, "asset_id", None)
                                        != asset_obj.uuid
                                    ):
                                    ...
                                    logger.warning(
                                        f"Impossible de lier l'asset '{asset_name}' à l'initiative '{name}': {e}"
                                    )
                            ...
                            for bi in budget_items:
                                ...
                                contributor_email = bi.get(
                                    "contributor_email"
                                ) or bi.get("contributor")
                                contributor = _get_user(
                                    contributor_email or admin_email_env
                                )
                                validator = None
                                if state in ["approved", "rejected"]:
                                    validator = _get_user(
                                        bi.get("validator_email") or admin_email_env
                                    )
                                ...
                                try:
                                    contributor_key = (
                                        contributor
                                        or TibilletUser.objects.filter(
                                    ...
                                    logger.warning(
                                        f"BudgetItem non créé pour '{name}': {e}"
                                    )
                            ...
                            if not budget_items and (goal_units or 0) > 0:
                                try:
                                    ...
                                    logger.warning(
                                        f"Impossible de générer des BudgetItem par défaut pour '{name}': {e}"
                                    )
                            ...
                            try:
                                ...
                                if provided_parts and not getattr(
                                    initiative_obj, "budget_contributif", False
                                ):
                                    initiative_obj.budget_contributif = True
                                    initiative_obj.save(
                                        update_fields=["budget_contributif"]
                                    )
                            except Exception:
                            ...
                            for p in init.get("participations", []) or []:
                                ...
                                user_email = (
                                    raw_email.replace("${ADMIN_EMAIL}", admin_email_env)
                                    if raw_email
                                    else admin_email_env
                                )
                                try:
                                ...
                                if part.state != state:
                                    part.state = state
                                    updates.append("state")
                                if time_spent is not None and state in (
                            ...
                            logger.warning(
                                f"Erreur lors de l'import de l'initiative '{init.get('name')}' pour {tenant.name}: {e}"
                            )
                            ...
                            for v in provided_votes:
                                ...
                                try:
                                    ...
                                    Vote.objects.get_or_create(
                                        initiative=initiative_obj, user=voter
                                    )
                                except Exception as e:
                                    logger.warning(
                                        f"Vote non créé pour '{initiative_obj.name}' ({u_email}): {e}"
                                    )
                    ...
                    for fed in feds:
                        try:
                            ...
                            if not target_client:
                                logger.warning(
                                    f"Federation ignorée: tenant cible '{target_name}' introuvable pour {tenant.name}"
                                )
                                continue
                            # Créer/mettre à jour la fédération
                            fp, created_fp = FederatedPlace.objects.get_or_create(
                                tenant=target_client
                            )
                            updates = []
                            ...
                            include_tags = [
                                t for t in (fed.get("include_tags") or []) if t
                            ]
                            exclude_tags = [
                                t for t in (fed.get("exclude_tags") or []) if t
                            ]
                            ...
                            logger.warning(
                                f"Erreur lors de la configuration d'une fédération pour {tenant.name}: {e}"
                            )
        ...
        try:
            ...
            logger.warning(
                f"Erreur lors de l'assignation aléatoire des origines utilisateur: {e}"
            )
            ...
            for tenant in created_tenants:
                with tenant_context(tenant):
                    ...
                    if config.module_caisse:
                        self.stdout.write(
                            f"Création des données POS pour {tenant.name}…"
                        )
