...
# Date de référence fixe pour les tests validate_new_booking (finding §13).
# reference_date = 2026-06-01 → horizon 28j → jusqu'au 2026-06-29.
# / Fixed reference date for validate_new_booking tests (finding §13).
...
# dans la fenêtre [date_from 00:00, date_to+1 00:00).
# / compute_open_intervals returns O = complement of merged ClosedPeriods
# / within window [date_from 00:00, date_to+1 00:00).
# ---------------------------------------------------------------------------
...
def test_compute_open_intervals_handles_null_end_date():
    ...
    O = compute_open_intervals(
        ...
        date_from=datetime.date(2026, 6, 18),
        date_to=datetime.date(2026, 6, 25),
        tz=PARIS_TZ,
    )
...
def test_generate_theoretical_slots_from_weekday_template():
    ...
    O = compute_open_intervals([], date, date, PARIS_TZ)
    entries = [_oe(0, datetime.time(9, 0), 60, 2)]   # MONDAY 09:00, 60 min, 2 slots

    W = generate_theoretical_slots(entries, O, date, date, PARIS_TZ)
...
def test_generate_theoretical_slots_excludes_closed_dates():
    ...
    O = compute_open_intervals(
        ...
        datetime.date(2026, 6, 1), datetime.date(2026, 6, 8), PARIS_TZ,
    )
    entries = [_oe(0, datetime.time(9, 0), 60, 1)]   # MONDAY

    W = generate_theoretical_slots(
        entries, O, datetime.date(2026, 6, 1), datetime.date(2026, 6, 8), PARIS_TZ,
    )
...
def test_generate_theoretical_slots_respects_date_to_boundary():
    ...
    O = compute_open_intervals(
        [], datetime.date(2026, 6, 1), datetime.date(2026, 6, 7), PARIS_TZ,
    )
    entries = [_oe(0, datetime.time(9, 0), 60, 1)]   # MONDAY

    W = generate_theoretical_slots(
        entries, O, datetime.date(2026, 6, 1), datetime.date(2026, 6, 7), PARIS_TZ,
    )
...
def test_generate_theoretical_slots_start_on_closed_day_bleed_into_open_day_is_excluded():
    ...
    O = compute_open_intervals(
        [_cp(datetime.date(2026, 6, 1), datetime.date(2026, 6, 1))],
        datetime.date(2026, 6, 1), datetime.date(2026, 6, 2), PARIS_TZ,
    )
    entries = [_oe(0, datetime.time(23, 30), 60, 1)]   # MONDAY 23:30

    W = generate_theoretical_slots(
        entries, O, datetime.date(2026, 6, 1), datetime.date(2026, 6, 2), PARIS_TZ,
    )
...
def test_generate_theoretical_slots_bleed_into_closed_day_start_date_is_open():
    ...
    O = compute_open_intervals(
        ...
        datetime.date(2026, 6, 7), datetime.date(2026, 6, 8), PARIS_TZ,
    )
    entries = [_oe(6, datetime.time(23, 30), 60, 1)]   # SUNDAY 23:30

    W = generate_theoretical_slots(
        entries, O, datetime.date(2026, 6, 7), datetime.date(2026, 6, 8), PARIS_TZ,
    )
...
def test_generate_theoretical_slots_multi_day_slot_all_open_days_is_returned():
    ...
    O = compute_open_intervals(
        [], datetime.date(2026, 6, 4), datetime.date(2026, 6, 6), PARIS_TZ,
    )
    entries = [_oe(3, datetime.time(0, 0), 2880, 1)]   # THURSDAY 00:00, 2880 min, 1 slot

    W = generate_theoretical_slots(
        entries, O, datetime.date(2026, 6, 4), datetime.date(2026, 6, 6), PARIS_TZ,
    )
...
def test_generate_theoretical_slots_three_day_slot_with_closed_middle_day_is_excluded():
    ...
    O = compute_open_intervals(
        ...
        datetime.date(2026, 6, 4), datetime.date(2026, 6, 7), PARIS_TZ,
    )
    entries = [_oe(3, datetime.time(0, 0), 4320, 1)]   # THURSDAY 00:00, 4320 min, 1 slot

    W = generate_theoretical_slots(
        entries, O, datetime.date(2026, 6, 4), datetime.date(2026, 6, 7), PARIS_TZ,
    )

    assert len(W) == 0


def test_group_id_same_for_slots_from_same_entry_and_date():
    """
    Un OpeningEntry slot_count=3 génère 3 créneaux qui partagent le même group_id.
    / One OpeningEntry slot_count=3 generates 3 slots sharing the same group_id.

    LOCALISATION : booking/tests/test_booking_engine.py

    OpeningEntry MONDAY "10:00" 60 3 → 10:00–11:00, 11:00–12:00, 12:00–13:00.
    Les trois créneaux proviennent du même groupe → group_id identique.
    / All three slots come from the same group → identical group_id.
    """
    from booking.booking_engine import compute_open_intervals, generate_theoretical_slots

    # Aucune période fermée — O couvre la journée entière du lundi 1er juin 2026.
    # / No closed period — O covers the full day of Monday June 1 2026.
    O = compute_open_intervals(
        [], datetime.date(2026, 6, 1), datetime.date(2026, 6, 1), PARIS_TZ,
    )
    entries = [_oe(0, datetime.time(10, 0), 60, 3)]   # MONDAY 10:00, 60 min, 3 slots

    W = generate_theoretical_slots(
        entries, O, datetime.date(2026, 6, 1), datetime.date(2026, 6, 1), PARIS_TZ,
    )

    assert len(W) == 3
    # Tous les créneaux appartiennent au même groupe.
    # / All slots belong to the same group.
    assert W[0].group_id == W[1].group_id == W[2].group_id


def test_group_id_differs_across_entries_and_across_weeks():
    """
    Deux OpeningEntry sur le lundi, fenêtre de deux semaines → 4 group_id distincts.
    / Two OpeningEntry on Monday, two-week window → 4 distinct group_id values.

    LOCALISATION : booking/tests/test_booking_engine.py

    entry_A : MONDAY 09:00 60 2 → 2 créneaux par lundi
    entry_B : MONDAY 14:00 60 2 → 2 créneaux par lundi
    Semaine 1 (Jun 1) + semaine 2 (Jun 8) → 8 créneaux au total.
    Groupes distincts : (A, Jun 1), (A, Jun 8), (B, Jun 1), (B, Jun 8) → 4 ids.
    / Distinct groups: (A, Jun 1), (A, Jun 8), (B, Jun 1), (B, Jun 8) → 4 ids.
    """
    from booking.booking_engine import compute_open_intervals, generate_theoretical_slots
    ...
    entries = [
        _oe(0, datetime.time(9, 0),  60, 2),   # MONDAY 09:00, 60 min, 2 slots
        _oe(0, datetime.time(14, 0), 60, 2),   # MONDAY 14:00, 60 min, 2 slots
    ]
    ...
    assert len(W) == 8
    # Quatre group_id distincts — un par (entry, semaine).
    # / Four distinct group_id values — one per (entry, week).
    group_ids = {slot.group_id for slot in W}
...
def test_compute_remaining_capacity_with_no_bookings_equals_capacity():
    ...
    O = compute_open_intervals([], date, date, PARIS_TZ)
    W = generate_theoretical_slots(
        [_oe(0, datetime.time(9, 0), 60, 1)], O, date, date, PARIS_TZ,
    )
    assert len(W) == 1
...
def test_compute_slots_booking_count_gt_1_overlaps_multiple_slots():
    ...
    with schema_context(TENANT_SCHEMA):
        try:
            ...
            # reference_date = Jun 01 = date_from → dans l'horizon par défaut.
            # / reference_date = Jun 01 = date_from → within default horizon.
            slots = compute_slots(
                resource,
...
def test_is_in_group_true_for_multi_slot_opening_entry():
    """
    OpeningEntry slot_count=3 → les 3 créneaux ont is_in_group=True.
    / OpeningEntry slot_count=3 → all 3 resulting slots have is_in_group=True.

    LOCALISATION : booking/tests/test_booking_engine.py

    Ressource avec un WeeklyOpening contenant un seul OpeningEntry lundi 10:00,
    durée 60 min, slot_count=3. compute_slots sur la semaine du 2026-06-01
    produit 3 créneaux → tous marqués is_in_group=True.
    / Resource with one OpeningEntry Monday 10:00, 60 min, slot_count=3.
    / compute_slots over week 2026-06-01 produces 3 slots → all is_in_group=True.
    """
    from booking.booking_engine import compute_slots
    from booking.views import annoter_creneaux_pour_affichage

    with schema_context(TENANT_SCHEMA):
        try:
            cal     = _make_calendar('is_in_group_multi')
            opening = _make_weekly_opening('is_in_group_multi')
            _add_opening_entry(opening, weekday=0, start_time=datetime.time(10, 0),
                               slot_duration_minutes=60, slot_count=3)
            ressource = _make_resource('is_in_group_multi', cal, opening, capacity=2, horizon=28)

            creneaux = annoter_creneaux_pour_affichage(compute_slots(
                ressource,
                date_from=datetime.date(2026, 6, 1),
            ...
            # L'entrée slot_count=3 génère 3 créneaux (un lundi dans la fenêtre).
            # / The slot_count=3 entry generates 3 slots (one Monday in the window).
            assert len(creneaux) == 3
            for creneau in creneaux:
                assert creneau.is_in_group is True

        finally:
            _cleanup()


def test_is_in_group_false_for_single_slot_opening_entry():
    """
    OpeningEntry slot_count=1 → le créneau unique a is_in_group=False.
    / OpeningEntry slot_count=1 → the single resulting slot has is_in_group=False.

    LOCALISATION : booking/tests/test_booking_engine.py

    Ressource avec un WeeklyOpening contenant un seul OpeningEntry lundi 10:00,
    durée 60 min, slot_count=1. compute_slots sur la semaine du 2026-06-01
    produit 1 créneau → marqué is_in_group=False.
    / Resource with one OpeningEntry Monday 10:00, 60 min, slot_count=1.
    / compute_slots over week 2026-06-01 produces 1 slot → is_in_group=False.
    """
    from booking.booking_engine import compute_slots
    from booking.views import annoter_creneaux_pour_affichage
    ...
    with schema_context(TENANT_SCHEMA):
        try:
            cal     = _make_calendar('is_in_group_single')
            opening = _make_weekly_opening('is_in_group_single')
            _add_opening_entry(opening, weekday=0, start_time=datetime.time(10, 0),
                               slot_duration_minutes=60, slot_count=1)
            ressource = _make_resource('is_in_group_single', cal, opening, capacity=2, horizon=28)
            ...
            # L'entrée slot_count=1 génère 1 créneau — pas de groupe.
            # / The slot_count=1 entry generates 1 slot — no group.
            assert len(creneaux) == 1
            assert creneaux[0].is_in_group is False
...
# ---------------------------------------------------------------------------
# Tests — compute_max_consecutive_slots  (unitaires / §10)
# pure :: [BookableInterval] → datetime → int → int
#
# Parcourt E' depuis start_datetime par pas de slot_duration_minutes et
# compte les créneaux consécutifs disponibles (remaining_capacity > 0).
# S'arrête dès qu'un créneau est absent ou complet.
# / Walks E' from start_datetime in steps of slot_duration_minutes and
# / counts consecutive available slots (remaining_capacity > 0).
# / Stops at the first missing or full slot.
# ---------------------------------------------------------------------------

def _bi(hour, minute, duration_minutes, remaining, capacity=10):
    """
    Crée un BookableInterval léger pour les tests unitaires purs.
    Date de référence fixe : 2026-06-08 (lundi dans REFERENCE_DATE + 7j).
    / Creates a lightweight BookableInterval for pure unit tests.
    Fixed reference date: 2026-06-08 (Monday within REFERENCE_DATE + 7d).
    """
    from booking.booking_engine import BookableInterval, Interval

    start = _aware(2026, 6, 8, hour, minute)
    end   = start + datetime.timedelta(minutes=duration_minutes)
    return BookableInterval(
...
def test_compute_max_consecutive_slots_all_available():
    """
    Trois créneaux consécutifs tous disponibles → retourne 3.
    / Three consecutive slots all available → returns 3.

    LOCALISATION : booking/tests/test_booking_engine.py

    Créneaux : 09:00, 10:00, 11:00 — remaining_capacity > 0 pour chacun.
    / Slots: 09:00, 10:00, 11:00 — remaining_capacity > 0 for each.
    """
    from booking.booking_engine import compute_max_consecutive_slots

    creneaux = [
        _bi(9,  0, 60, remaining=5),
        _bi(10, 0, 60, remaining=5),
        _bi(11, 0, 60, remaining=5),
    ]
    start = _aware(2026, 6, 8, 9, 0)

    assert compute_max_consecutive_slots(creneaux, start, 60) == 3


def test_compute_max_consecutive_slots_stops_at_full_second_slot():
    """
    Trois créneaux, le 2e est complet → retourne 1.
    / Three slots, 2nd is full → returns 1.

    LOCALISATION : booking/tests/test_booking_engine.py

    Créneaux : 09:00 (dispo), 10:00 (complet), 11:00 (dispo).
    compute_max_consecutive_slots s'arrête au premier créneau complet.
    / Slots: 09:00 (available), 10:00 (full), 11:00 (available).
    Function stops at the first full slot.
    """
    from booking.booking_engine import compute_max_consecutive_slots

    creneaux = [
        _bi(9,  0, 60, remaining=5),
        _bi(10, 0, 60, remaining=0),
        _bi(11, 0, 60, remaining=5),
    ]
    start = _aware(2026, 6, 8, 9, 0)

    assert compute_max_consecutive_slots(creneaux, start, 60) == 1


def test_compute_max_consecutive_slots_returns_zero_when_first_slot_full():
    """
    Premier créneau complet → retourne 0.
    / First slot full → returns 0.
    ...
    creneaux = [
        _bi(9,  0, 60, remaining=0),
        _bi(10, 0, 60, remaining=5),
    ]
    start = _aware(2026, 6, 8, 9, 0)

    assert compute_max_consecutive_slots(creneaux, start, 60) == 0


def test_compute_max_consecutive_slots_returns_zero_when_start_not_in_creneaux():
    """
    start_datetime absent de E' → retourne 0.
    / start_datetime not in E' → returns 0.

    LOCALISATION : booking/tests/test_booking_engine.py

    Les créneaux commencent à 10:00 mais on demande à partir de 09:00 —
    absent de la liste → 0 immédiatement.
    / Slots start at 10:00 but start requested at 09:00 —
    not in the list → 0 immediately.
    """
    from booking.booking_engine import compute_max_consecutive_slots
...
def test_compute_max_consecutive_slots_stops_at_gap_in_sequence():
    """
    Séquence avec un trou (09:00, 11:00 — 10:00 manquant) → retourne 1.
    / Sequence with a gap (09:00, 11:00 — 10:00 missing) → returns 1.

    LOCALISATION : booking/tests/test_booking_engine.py

    Le créneau 10:00 n'existe pas dans E' — la fonction doit s'arrêter
    même si 11:00 est disponible.
    / The 10:00 slot doesn't exist in E' — the function must stop
    even though 11:00 is available.
    """
    from booking.booking_engine import compute_max_consecutive_slots

    creneaux = [
        _bi(9,  0, 60, remaining=5),
        # 10:00 manquant / 10:00 missing
        _bi(11, 0, 60, remaining=5),
    ]
    start = _aware(2026, 6, 8, 9, 0)
...
def test_compute_max_consecutive_slots_returns_zero_on_empty_list():
    """
    Liste de créneaux vide → retourne 0.
    / Empty slot list → returns 0.
    ...
    assert compute_max_consecutive_slots([], start, 60) == 0
