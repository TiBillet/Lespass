...
|--------------------|---------------------|----------|----------------------------------------------------------------|
| `confirmed_bookings` | `QuerySet[Booking]` | yes    | Member's upcoming `confirmed` bookings, ordered by `start_datetime`. Filtered: `user=request.user`, `status='confirmed'`, `start_datetime__gt=now()`. `select_related('resource')` applied. |
