...
export const sessionActions = {
  ...
  async emailActivation(id, token) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        log({
          message:
            "sessionStore -> emailActivation /api/user/activate/, status = 200",
          object: retour,
        });
  ...
  async automaticConnection() {
    ...
    if (refreshToken !== undefined && refreshToken !== "") {
      ...
      try {
        ...
        log({
          message: "automaticConnection, /api/user/token/refresh/, error:",
          error,
        });
        emitter.emit("toastSend", {
  ...
  resetEventOptions() {
    let form = this.forms.find(
      (formRec) => formRec.uuid === this.currentUuidEventForm
    );
    form.options_checkbox.forEach((option) => {
      option.checked = false;
    });
    document
      .querySelectorAll('input[name="event-option-radio"]')
      .forEach((ele) => {
        ele.checked = false;
      });
  ...
  initFormEvent(postEvent) {
    ...
    const returnString =
      JSON.stringify(postEvent) + JSON.stringify(this.membershipProducts);
    ...
    if (form === undefined || form.eventHash !== hash) {
      ...
      postEvent.options_checkbox.forEach(
      ...
      postEvent.products.forEach((product) => {
        ...
        product.prices.forEach((price) => {
          ...
          if (price.adhesion_obligatoire !== null) {
            let newProduct = this.membershipProducts.find(
              (membership) => membership.uuid === price.adhesion_obligatoire
            );
            ...
            newProduct["priceLinkWithMembership"] = {
          ...
          if (
            product.categorie_article !== "D" &&
            product.categorie_article !== "S"
          ) {
  ...
  resetPriceCustomers(product) {
    ...
    let products = this.forms.find(
      (form) => form.uuid === this.currentUuidEventForm
    ).products;
    let prices = products.find(
      (product) => product.uuid === price.productUuid
    ).prices;
  ...
  deleteEventForm(uuid) {
    if (
      this.forms.find((form) => form.uuid === uuid).typeForm === "reservation"
    ) {
      this.router.push("/");
      console.log("reset formulaire !");
      // suppression du formulaire "reservation" courant
      const newForms = this.forms.filter(
        (form) => form.uuid !== this.currentUuidEventForm
      );
      ...
      log({
        message: `sessionStore -> deleteCurrentEventForm, ce n'est pas un formulaire de réservation`,
      });
    }
  },
  ...
  activationProductMembership(price) {
    ...
    const products = this.forms.find(
      (form) => form.uuid === this.currentUuidEventForm
    ).products;
...
*/
    ...
    this.loading = true;
    try {
      const response = await fetch(domain + apiStripe, options);
      ...
      if (response.status === 200) {
        ...
        emitter.emit("toastSend", {
          title: "Attention",
          contenu: `Erreur lors de la création du compte stripe, recommencer SVP.`,
          typeMsg: "warning",
          delay: 10000,
        });
      }
  ...
  async postStripeReturn(uuidStripe) {
    ...
    fetch(domain + apiStripe, options)
      ...
      .catch(function (error) {
        log({
          message: "postStripeReturn, /api/webhook_stripe/ error: ",
          error,
        });
        emitter.emit("modalMessage", {
          titre: "Erreur",
  ...
  addCustomer(productUuid, priceUuid) {
    ...
    let customers = product.prices.find(
      (prix) => prix.uuid === priceUuid
    ).customers;
    customers.push({
      first_name: "",
      last_name: "",
