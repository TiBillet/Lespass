<template>
  <div class="input-group input-group-dynamic mb-4" :class="validation === true ? 'has-validation' : ''">
    ...
    <textarea :id="id" :rows="rowsMd" class="form-control tibillet-textarea-md" :value="modelValue"
      @input="sendInput($event)" aria-describedby="basic-addon3" @focusin="focused($event)" @focusout="defocused($event)"
      @keyup="isFilled($event)" :required="validation" role="textbox" :aria-label="msgRole"></textarea>
