...
**Quoi / What :** le module Newsletter s'active désormais comme les autres, depuis le dashboard,
par n'importe quel gestionnaire du lieu. Le verrou « superadmin seulement » (modale
« contactez-nous » + POST refusé en 403) est supprimé.

**Pourquoi / Why :** la contrainte « il faut un serveur Ghost » n'est pas un verrou technique à
imposer dans le code. On la rappelle simplement, là où elle est utile : sur la carte du dashboard
et sur la page de configuration Ghost, avec une invitation à contacter l'équipe TiBillet (qui peut
héberger l'instance).

### Fichiers modifiés / Modified files
| Fichier / File | Changement / Change |
|---|---|
| `Administration/admin/dashboard.py` | Retrait du flag `superadmin_seulement` du module ; mention « serveur Ghost / TiBillet peut vous en héberger » ajoutée à la description de la carte |
| `Administration/admin_tenant.py` | Retrait des deux branches `superadmin_seulement` (`module_toggle_modal` et `module_toggle`) |
| `Administration/templates/admin/ghost/panneau_newsletter.html` | Mention d'hébergement ajoutée en section « 1. La connexion » |
| `Administration/templates/admin/dashboard_module_modal_contact.html` | **Supprimé** (modale de contact devenue inutile) |
| `tests/pytest/test_module_newsletter_activation.py` | Tests réécrits : le module s'active par tout gestionnaire (5 tests verts) |
...
## L'imprimante appartient au terminal, plus au point de vente / The printer belongs to the terminal, no longer to the point of sale

**Date :** 2026-07-14
**Migration :** **Oui** — `laboutik/0002` et `kiosk/0005`.
`migrate_schemas --executor=multiprocessing`
**Spec :** [`TECH_DOC/SESSIONS/IMPRESSION/`](TECH_DOC/SESSIONS/IMPRESSION/INDEX.md)

### 1. Un ticket sortait en autant d'exemplaires qu'il y avait de tablettes

**Quoi / What :** l'imprimante du ticket client était portée par `PointDeVente.printer`. Or un
point de vente n'est **pas** un terminal : en festival, une vingtaine de tablettes encaissent sur
le même point de vente « Bar », chacune avec sa propre imprimante Sunmi intégrée.

**Pourquoi / Why :** la vue injectait `state["printer"] = pv.printer`, et chaque tablette s'abonnait
au canal WebSocket `ws/printer/<uuid>/` correspondant. Les vingt tablettes rejoignaient donc **le
même groupe Redis** : chaque ticket envoyé était reçu par les vingt, et **s'imprimait vingt fois**.

L'imprimante est désormais portée par un objet **`Terminal`**, et une seule règle de résolution
gouverne tout ce qu'un terminal imprime (ticket de vente, billet, reçu, ticket X, ticket Z) :
`request.user.terminal.printer`.

Ce n'est pas une nouveauté : LaBoutik V1 faisait déjà porter le reçu client par le terminal
(`Printer.host` → `Appareil`). La réécriture V2 a supprimé le modèle `Appareil` et `PointDeVente`
en a hérité le rôle, sans arbitrage écrit. Ce chantier restaure ce porteur.

### 2. `kiosk.Terminal` devient `laboutik.Terminal` — le TPE se libère du kiosque

**Quoi / What :** le modèle « appareil appairé » **existait déjà**, dans `kiosk`. Il portait le nom,
l'UUID et le lien vers le compte (`term_user`). Il ne lui manquait qu'une imprimante.

**Pourquoi / Why :** un TPE Stripe n'est pas propre aux bornes libre-service — une caisse LaBoutik
peut en avoir un. Et surtout, en créer un second aurait été **impossible** : deux
`related_name="terminal"` sur `TibilletUser` déclenchent un `fields.E304` et Django refuse de
démarrer.

`Terminal` porte donc maintenant des **capacités optionnelles** : il imprime (`printer`), il
encaisse par carte (`registration_code`, `stripe_id`), ou les deux. Le défaut du champ `type` passe
de « TPE WisePOS » à « **Pas de TPE** » : le cas courant est une caisse qui imprime.

### 3. Révoquer un terminal — le mécanisme n'existait pas

**Quoi / What :** une action d'admin « Révoquer le terminal » sur `TerminalAdmin`.

**Pourquoi / Why :** chaque ré-appairage crée un nouveau `PairingDevice`, donc un nouveau
`TermUser` et une nouvelle clé — **pendant que les anciens restent actifs**. Rien ne les coupait.
L'action tire les **deux** leviers, et les deux sont nécessaires : `term_user.is_active = False`
(coupe le pont d'authentification et les reconnexions WebSocket) **et**
`LaBoutikAPIKey.revoked = True` (coupe l'en-tête Api-Key — la clé est stockée sur l'appareil, sans
la révoquer il suffirait de réactiver le compte).

### Fichiers modifiés / Modified files

| Fichier / File | Changement / Change |
|---|---|
| `laboutik/models.py` | **Nouveau** `Terminal` (+ `StripeLocation`, déplacés depuis `kiosk`). **Suppression** de `PointDeVente.printer` |
| `laboutik/views.py` | **Nouvelle** fonction `imprimante_du_terminal(user)` — la règle unique. Bascule des 7 sites d'impression + `state["printer"]` (l'abonnement WebSocket) + 5 `select_related("printer")` |
| `kiosk/models.py` | `Terminal` et `StripeLocation` partis vers `laboutik`. `PaymentsIntent.terminal` repointé |
| `kiosk/admin.py` | `TerminalForm` / `TerminalAdmin` partis vers `Administration/admin/laboutik.py` |
| `Administration/admin/laboutik.py` | **Nouveau** `TerminalAdmin` (+ action « Révoquer »). Colonne « Terminaux » sur `PrinterAdmin`. Champ `printer` retiré du `PointDeVenteAdmin` |
| `discovery/views.py` | Le claim crée le `Terminal` |
| `laboutik/management/commands/create_test_pos_data.py` | L'imprimante mock se rattache aux terminaux, plus aux points de vente |
| `Administration/admin/dashboard.py` | Lien de la sidebar : `kiosk_terminal_changelist` → `laboutik_terminal_changelist` |
| `tests/pytest/test_impression_par_terminal.py` | **Nouveau** — 9 tests, dont celui qui prouve la correction |

### Attention / Heads-up

**Une session humaine n'imprime plus le ticket client.** Un gestionnaire connecté dans un
navigateur n'a pas de `Terminal`, donc pas d'imprimante. C'est cohérent — une imprimante appartient
à un appareil, pas à un onglet — mais c'est un changement de comportement visible. Pour imprimer,
il faut un appareil appairé.

**Les `PaymentsIntent` existants ont été purgés** par la migration `kiosk/0005` : ce sont des traces
d'événements Stripe (le crédit réel vit dans Fedow), et rien n'est en production.
...
## Merge `main` → `main-fedow-import` : Newsletter rejoint Fedow V2 / Merge: Newsletter meets Fedow V2

**Date :** 2026-07-14
**Migration :** **Oui** — `BaseBillet.0229_merge_20260714_1045` (migration de merge, vide)

**Quoi / What :** reunion du chantier **Newsletter / Ghost** (`main`, 8 commits) et du chantier
**Fedow V2 / LaBoutik / Kiosk / Pages** (`main-fedow-import`, 90 commits). Les deux modules de
`Configuration` coexistent desormais : `module_pages` **et** `module_newsletter`.

**Pourquoi / Why :** les deux branches divergeaient trop pour continuer separement.

### Ce que le merge a exige, au-dela des marqueurs de conflit

Git signalait 8 conflits. Deux problemes de fond, eux, **ne levaient aucun marqueur** :

1. **Deux feuilles concurrentes dans le graphe de migrations.** `main` apportait
   `0221_configuration_module_newsletter` (branchee sur `0220`), quand `main-fedow-import` etait
   rendue a `0228`. Django refusait de demarrer (`Conflicting migrations detected`). Reunies par
   `makemigrations --merge`.

2. **Un template orphelin dans une arborescence morte.** `main-fedow-import` a reorganise les
   templates (`reunion/` → `commun/` + `fonctionnel/`) ; `main`, restee sur l'ancienne arbo, y
   avait **ajoute** `error_useful_links.html`. Git l'a laisse la — seul survivant d'un dossier
   par ailleurs supprime — et `404.html` / `500.html`, **fusionnes sans conflit donc en
   silence**, continuaient de l'inclure depuis ce chemin mort. Deplace vers `commun/partials/`.

### Un bug de test revele (et non cause) par le merge

`test_federation_tags_semantique.py` (de `main`) colle la connexion sur `lespass` via un
`DjangoClient(HTTP_HOST=…)` — le middleware django-tenants ne la decolle jamais. Or
`test_fedow_core.py` (de `main-fedow-import`) teste des SHARED_APPS et **suppose** le schema
`public`. L'ordre de collecte alphabetique les met cote a cote (« fede… » < « fedo… ») :
**8 tests rouges**. Sur `main`, le pollueur n'avait **aucune victime** — le fichier n'y existe
pas. Le bug etait invisible des deux cotes.

Corrige dans la **victime**, qui pose `public` en **setup** (jamais en teardown : les finalizers
des fixtures de portee superieure s'executent apres et tomberaient sur `public`).

Suite complete : **834 passed, 0 echec**.

### Fichiers modifies / Modified files
| Fichier / File | Changement / Change |
|---|---|
| `TiBillet/settings.py` | Union des `INSTALLED_APPS` : apps fedow **et** `newsletter` |
| `BaseBillet/models.py` | `Configuration` : `module_pages` **et** `module_newsletter` |
| `Administration/admin_tenant.py` | Import a effet de bord unifie (`# noqa: F401`), etendu a `laboutik` + `inventaire`, plus `pages.admin` |
| `Administration/admin/dashboard.py` | Les deux cartes de module. Ancienne entree « Ghost » d'« Outils externes » retiree : `main` l'a deplacee dans la section « Newsletter » |
| `BaseBillet/views.py` | Chemins de templates de l'arbo cible (`commun/adhesion/`) |
| `BaseBillet/templates/commun/partials/error_useful_links.html` | **Deplace** depuis `reunion/partials/` ; includes de `404.html` / `500.html` repointes |
| `BaseBillet/migrations/0229_merge_20260714_1045.py` | **Nouveau** — merge technique des feuilles `0221` / `0228` (vide) |
| `tests/pytest/test_fedow_core.py` | Fixture autouse module-scoped : `public` en setup |
| `tests/PIEGES.md` | Nouveau piege — un merge peut **reveler** une fuite de schema sans la causer |
| `locale/{fr,en}/LC_MESSAGES/django.po` | Repris de `main` — **a regenerer** (ils ne portent pas les chaines de `main-fedow-import`) |
...
## Controlvanne : autocomplétion de la carte de maintenance / Controlvanne: maintenance card autocompletion

**Date :** 2026-07-12
**Migration :** Non / No
**Issue :** [#446](https://github.com/TiBillet/Lespass/issues/446)

**Quoi / What :** le champ « Carte NFC » du formulaire d'ajout d'une carte de maintenance
(`/admin/controlvanne/cartemaintenance/add/`) passe de `raw_id_fields` à `autocomplete_fields`.
On tape désormais le **Tag ID** lu sur la puce (ou le numéro imprimé) et les résultats arrivent
en direct, au lieu de devoir saisir le **pk numérique** de la carte.

Le même correctif ajoute un `formfield_for_foreignkey` qui restreint les cartes sélectionnables
à celles du lieu courant (`detail__origine`).

**Pourquoi / Why :** en régie, la carte est identifiée par son Tag ID, jamais par son pk. Le champ
raw_id ne reconnaissait donc rien, et sa loupe ouvrait le changelist des cartes en popup — pris pour
un formulaire de création. La docstring de l'admin annonçait déjà `autocomplete_fields` : le code ne
faisait pas ce que le commentaire promettait.

**Fuite cross-tenant corrigée au passage :** `CarteCashless` est en SHARED_APPS (schéma `public`,
aucune isolation). Sans queryset filtré, le champ proposait — et **validait** — les cartes de tous
les lieux. Un pk forgé aurait rattaché la carte d'un autre lieu comme carte de maintenance.
`CartePrimaireAdmin` (laboutik) documente ce piège ; `CarteMaintenanceAdmin` avait copié le patron
sans la protection.

### Fichiers modifiés / Modified files
| Fichier / File | Changement / Change |
|---|---|
| `controlvanne/admin.py` | `CarteMaintenanceAdmin` : `raw_id_fields` → `autocomplete_fields` ; ajout de `formfield_for_foreignkey` filtrant sur le lieu courant ; imports `connection` + `CarteCashless` |
| `tests/pytest/test_controlvanne_admin_carte_maintenance.py` | **Nouveau** — 2 tests : widget autocomplete, isolation cross-tenant |
| `A TESTER et DOCUMENTER/controlvanne-carte-maintenance-autocompletion.md` | **Nouveau** — scénarios de recette |
