...
### 4.2 — Bugs UX constatés lors de la review visuelle (Session 02)

Problèmes identifiés en naviguant sur la page bilan avec Chrome, à corriger dans cette session.

**Corrigés pendant la Session 02 :**
- [x] Canvas Chart.js vide prenait ~400px de hauteur quand l'event n'a pas de données → ajout d'un booléen `has_courbe_ventes` dans le contexte (remplace la comparaison fragile de strings JSON)
- [x] Section Scans affichée avec 0/0/0 quand aucun billet → masquée via `has_scans`
- [x] Chart.js ignorait `height="200"` (mode responsive ratio 2:1) → wrapper `<div style="position: relative; height: 250px;">` pour contraindre la hauteur

**À corriger dans cette session :**
- [ ] **Texte "Fill rate: X%" redondant** sous la progress bar — le pourcentage est déjà affiché à droite de la barre. Supprimer la ligne `Fill rate:` en dessous.
- [ ] **Dates axe X au format ISO** ("2026-04-01") — le service `calculer_courbe_ventes()` utilise `strftime('%d/%m')` mais Chart.js affiche les dates brutes. Vérifier le format des labels dans le JSON passé au chart.
- [ ] **Mélange FR/EN dans les en-têtes de tableaux** — "Tarif", "Sold", "Free", "Revenue incl. tax", "Remboursé" sur la même ligne. L'i18n n'est pas appliquée uniformément. Passer tous les en-têtes dans `{% translate %}`.
- [ ] **Symbole € manquant** sur certains montants dans les tableaux (ex: "30,00 €" dans synthèse mais "30,00 €" dans le tableau tarifs — vérifier la cohérence)
- [ ] **Pas de ligne TOTAL** dans le tableau "Sales by rate" — la spec prévoit une ligne de total en gras fond gris en bas du tableau

---

### 4.3 — Vérification a11y (renumerotée, était 4.2)
