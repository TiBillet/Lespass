...
#
# Usage : scripts/lancer_tests.sh <python|e2e> <sans-stripe|stripe> [arguments pytest…]
...
SUITE="${1:-}"
MODE="${2:-}"
if [ "$SUITE" != "python" ] && [ "$SUITE" != "e2e" ]; then
    echo "Usage : $0 <python|e2e> <sans-stripe|stripe> [arguments pytest…]" >&2
    exit 2
fi
