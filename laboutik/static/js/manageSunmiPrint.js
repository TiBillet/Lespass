...
async function sunmiInnerPrinterIsOn() {
  try {
    const sunmiService = await SunmiPrinterPlugin.initSunmiPrinterService()
    const printerAvailable = await SunmiPrinterPlugin.isPrinterAvailable()
    if (sunmiService === 'Printer connected' && printerAvailable === 'enabled') {
      return true
    } else {
      return false
    }
  } catch (error) {
    console.log('-> sunmiInnerPrinterIsOn,', error)
    return false
  }
}
...
async function initWebsocket() {
  ...
  // state.printer = imprimente ajoutée dans l'admin.
  // Si pas ajouter, même si inner printer = pas d'impression
