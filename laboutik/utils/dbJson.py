		...
		'memberships': [
			{
				...
				"name": 'Panaier amap1 1 mois',
				"range": ['14/11/2025', '15/12/2025']
			},
			{
				"uuid": 'ghxsghggsyulmsj65hhkjsztks',
				"name": 'Adhésion lieu',
				"range": ['14/11/2025', '15/12/2026']
			}
		]
	},
  {
...
class mockDb:
  ...
  def del_by_id(self, table, value):
    try:
      db_table = self.data[table]
      finds = [item for item in db_table if item['id'] == value]
      print(f"del_by_id, finds = {finds}")
      self.data[table].remove(finds[0])
      self.write_json(self.data)
      return True
    except Exception as error:
      print(f"save db, error: {error}")
  ...
  def get_by_index(self, table, index, value):
    try:
      db_table = self.data[table]
      finds = [item for item in db_table if item[index] == value]
      # print(f"db_table = {db_table}")
      return finds
    except Exception as error:
      print(f"get_by_index db, error: {error}")
      return None
  ...
  def get_all(self, table):
    try:
      ...
      print(f"get_all db, error: {error}")
      return None
  ...
  def del_table(self, table):
    try:
      ...
      print(f"del_table db, error: {error}")
      return False
  ...
  def create_table(self, table_name):
    try:
      ...
      return self.write_json(self.data)
    except Exception as error:
      print(f"create_table db, error: {error}")
      return False
...
# Code d'exécution supprimé — utiliser mockData.mockDb à la place