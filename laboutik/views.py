...
class CaisseViewSet(viewsets.ViewSet):
    ...
    def point_de_vente(self, request):
        ...
        if point_de_vente["comportement"] == "K":
            template_name = "laboutik/views/kiosk.html"

        # Enrichir le state avec les propriétés du point de vente
        # (transmis au JS via stateJson pour piloter l'interface)
        # Enrich state with the point of sale properties
        # (passed to JS via stateJson to drive the interface)
...
class PaiementViewSet(viewsets.ViewSet):
    ...
    def payer(self, request):
        ...
        - CB ou chèque → succès immédiat / immediate success
        - Espèce → vérifie la somme donnée, calcule la monnaie à rendre / checks given sum, calculates change
        - NFC (cashless) → vérifie le solde de la carte, fonds insuffisants si pas assez
                           checks card balance, insufficient funds if not enough
        """
        state = _construire_state()
        db = _charger_mock_db()
        tous_les_points_de_vente = _charger_points_de_vente()
        ...
        # Convertir les montants en centimes (entiers)
        ...
        donnees_paiement["given_sum"] = 0 if somme_donnee_brute == "" else int(somme_donnee_brute)
        donnees_paiement["missing"] = 0

        # Vérifier si une consigne est dans le panier (flux de remboursement)
        # Check if a deposit is in the basket (refund flow)
        consigne_dans_panier = UUID_ARTICLE_CONSIGNE in uuids_articles_selectionnes
        ...
        # Chercher une transaction précédente (cas "fonds insuffisants" avec complément)
        # Look for a previous transaction (case "insufficient funds" with top-up)
        ...
        # Contexte de base pour les templates de retour
        # Base context for return templates
        ...
        # --- Paiement par carte bancaire ou chèque ---
        # --- Credit card or check payment ---
        if moyen_paiement_code in ("carte_bancaire", "CH"):
            # Convertir le total en euros pour l'affichage (centimes → euros)
            # Convert total to euros for display (cents → euros)
        ...
        if moyen_paiement_code == "espece":
            # Si c'est un complément (après fonds insuffisants), prendre en compte le premier versement
            # If it's a top-up (after insufficient funds), account for the first payment
            ...
            somme_donnee = donnees_paiement["given_sum"]
            total_en_centimes = donnees_paiement["total"]

            # Somme suffisante (ou somme exacte non saisie = paiement exact)
            # Sufficient amount (or no amount entered = exact payment)
            if somme_donnee == 0 or (somme_donnee + montant_premier_versement) >= total_en_centimes:
                donnees_paiement["give_back"] = 0
                if somme_donnee > total_en_centimes:
                    # Calculer la monnaie à rendre (en euros)
                    # Calculate change to give back (in euros)
                    donnees_paiement["give_back"] = (somme_donnee - total_en_centimes) / 100
                donnees_paiement["total"] = total_addition
                return render(request, "laboutik/partial/hx_return_payment_success.html", context)

        # --- Paiement NFC / cashless ---
        # --- NFC / cashless payment ---
        if moyen_paiement_code == "nfc":
            ...
            # Chercher la carte NFC du client dans la base mock
            # Look up the client's NFC card in the mock database
            tag_id_client = donnees_paiement.get("tag_id", "")
            cartes_trouvees = db.get_by_index("cards", "tag_id", tag_id_client)

            # Carte inconnue → avertissement
            ...
            solde_total_carte = carte_client["wallets"] + carte_client["wallets_gift"]

            # Récupérer les moyens de paiement acceptés par le PV (pour le fallback fonds insuffisants)
            # Get the payment methods accepted by the POS (for insufficient funds fallback)
            ...
            context_nfc = {
                ...
                "currency_data": CURRENCY_DATA,
                "wallets": {
                    "monnaie": carte_client["wallets"],
                    "gift_monnaie": carte_client["wallets_gift"],
                },
                "payments_accepted": {
            ...
            # Fonds insuffisants → sauvegarder la transaction et proposer un complément
            # Insufficient funds → save the transaction and offer a top-up
