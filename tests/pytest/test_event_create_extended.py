...
def test_event_create_with_schema_org_fields(request, api_client, auth_headers):
    ...
    assert resp.status_code == 201, f"HTTP {resp.status_code}: {resp.content.decode()[:500]}"
    data = resp.json()
    ...
    # Lecture
    detail = api_client.get(f'/api/v2/events/{ev_id}/', **auth_headers)
    assert detail.status_code == 200, f"HTTP {detail.status_code}: {detail.content.decode()[:500]}"
    det = detail.json()
    ...
    assert resp2.status_code == 201, f"Action create failed: {resp2.status_code} {resp2.content.decode()[:300]}"
    data2 = resp2.json()
