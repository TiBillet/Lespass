...
def cv_tireuse_sans_fut(tenant):
    ...
    with schema_context(tenant.schema_name):
        ...
        # test_cloture_caisse.py) qui prend le 1er PV non masque par poid_liste
        # — meme pattern que tireuse_billing dans test_controlvanne_billing.py.
        # / post_save signal auto-creates a non-hidden PointDeVente. Hide it so
        # it doesn't pollute premier_pv (test_menu_ventes.py, test_cloture_caisse.py)
        # which takes the 1st non-hidden PV by poid_liste — same pattern as
        # tireuse_billing in test_controlvanne_billing.py.
        if t.point_de_vente:
            from laboutik.models import PointDeVente

            PointDeVente.objects.filter(pk=t.point_de_vente_id).update(hidden=True)
        yield t
...
def cv_tireuse_avec_fut(tenant):
    ...
    with schema_context(tenant.schema_name):
        ...
        # Masquer le PointDeVente auto-cree — voir commentaire dans
        # cv_tireuse_sans_fut ci-dessus.
        # / Hide the auto-created PointDeVente — see comment in
        # cv_tireuse_sans_fut above.
        if t.point_de_vente:
            from laboutik.models import PointDeVente
...
class TestAuthorizeMaintenance:
    def test_08_authorize_carte_maintenance(
        self, tenant, cv_client, cv_headers, cv_tireuse_avec_fut, cv_carte_maintenance
    ):
        """Carte maintenance → is_maintenance=True, allowed_ml=reservoir.

        ADAPTATION vs source lespass-main : notre viewsets.py exige desormais
        tireuse.enabled=False pour autoriser une carte maintenance (regle de
        securite ajoutee depuis : une carte maintenance ne doit pas fonctionner
        en service normal, sinon elle serait utilisable comme carte gratuite).
        On desactive donc la tireuse avant l'appel — meme intention de test
        (verifier authorized=True + is_maintenance=True + allowed_ml=reservoir),
        juste adaptee a la nouvelle precondition.
        / ADAPTATION vs lespass-main source: our viewsets.py now requires
        tireuse.enabled=False to authorize a maintenance card (security rule
        added since: a maintenance card must not work during normal service,
        otherwise it would be usable as a free card). We disable the tap
        before the call — same test intent, just adapted to the new
        precondition.
        """
        with schema_context(tenant.schema_name):
            cv_tireuse_avec_fut.enabled = False
            cv_tireuse_avec_fut.save(update_fields=["enabled"])
        ...
        # Remettre enabled pour ne pas perturber les tests suivants (pour_update,
        # card_removed) qui utilisent cv_tireuse_avec_fut avec une carte normale.
        # / Restore enabled so subsequent tests (pour_update, card_removed) using
        # cv_tireuse_avec_fut with a normal card are not affected.
        with schema_context(tenant.schema_name):
            cv_tireuse_avec_fut.enabled = True
            cv_tireuse_avec_fut.save(update_fields=["enabled"])
