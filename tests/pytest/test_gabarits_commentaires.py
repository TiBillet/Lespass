...
PORTÉE
------
Cadré sur `pages/`, l'app des skins. Un passage sur tout le dépôt trouve 8 cas de
plus (emails BaseBillet + crowds + onboard, laboutik, admin Administration), tous de
la variété « jetée en silence ». Élargir ce test à `RACINE_DU_DEPOT` suppose de les
corriger d'abord : c'est une décision mainteneur.
/ Scoped to `pages/`, the skins app. A repo-wide pass finds 8 more, all of the
silently-discarded kind. Widening the scope means fixing those first — a maintainer
decision.
"""
