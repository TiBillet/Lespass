...
def test_membership_create_and_retrieve(api_client, auth_headers):
    ...
    membership_resp = api_client.post(
        '/api/v2/memberships/',
    ...
    assert membership_resp.status_code == 201, (
        f"Membership create failed: {membership_resp.status_code} {membership_resp.content.decode()[:300]}"
    )
    ...
    # Retrieve Membership
    det_resp = api_client.get(f'/api/v2/memberships/{membership_uuid}/', **auth_headers)
    assert det_resp.status_code == 200, f"Membership retrieve failed: {det_resp.status_code} {det_resp.content.decode()[:300]}"
