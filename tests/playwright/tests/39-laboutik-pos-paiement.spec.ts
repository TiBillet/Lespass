...
`);
    ...
    // Préparer CLIENT2 avec un wallet à solde insuffisant (1 centime)
    // Setup CLIENT2 with an insufficient balance wallet (1 cent)
    const setup2Result = djangoShell(`
from AuthBillet.models import Wallet
from QrcodeCashless.models import CarteCashless
from Customers.models import Client
...
wallet_client2, _ = Wallet.objects.get_or_create(name='[pw_test] Client2 NFC')
carte2 = CarteCashless.objects.get(tag_id='${DEMO_TAGID_CLIENT2}')
carte2.wallet_ephemere = wallet_client2
carte2.user = None
carte2.save(update_fields=['wallet_ephemere', 'user'])
Token.objects.filter(wallet=wallet_client2, asset=asset_tlf).delete()
with db_transaction.atomic():
    WalletService.crediter(wallet=wallet_client2, asset=asset_tlf, montant_en_centimes=1)
solde2 = WalletService.obtenir_solde(wallet=wallet_client2, asset=asset_tlf)
print(f'SETUP2_OK solde={solde2}')
`);
    console.log('NFC CLIENT2 setup:', setup2Result);
    expect(setup2Result).toContain('SETUP2_OK');
  });
...
`);
   ...
   *
   * CLIENT3 (D74B1B5D) n'est pas créé par create_test_pos_data mais apparaît
   * dans les boutons de simulation. Cliquer dessus doit afficher "Carte inconnue".
   *
   * CLIENT3 (D74B1B5D) is not created by create_test_pos_data but appears
   * in the simulation buttons. Clicking it should display "Carte inconnue".
   */
  test('paiement NFC carte inconnue / NFC payment with unknown card', async ({ page }) => {
    ...
    await test.step('Cliquer Carte client 3 (inexistante) / Click Carte client 3 (nonexistent)', async () => {
      // CLIENT3 tag D74B1B5D n'existe pas en base
      // CLIENT3 tag D74B1B5D does not exist in DB
      const client3Btn = page.locator('.nfc-reader-simu-bt[tag-id="D74B1B5D"]');
      await expect(client3Btn).toBeVisible();
      await client3Btn.click();
  ...
  test('paiement NFC solde insuffisant / NFC payment with insufficient balance', async ({ page }) => {
    ...
    await test.step('Cliquer Carte client 2 (solde insuffisant) / Click Carte client 2 (insufficient balance)', async () => {
      // CLIENT2 n'a que 1 centime — le produit coute plus cher
      // CLIENT2 only has 1 cent — the product costs more
      await page.locator(`.nfc-reader-simu-bt[tag-id="${DEMO_TAGID_CLIENT2}"]`).click();
...
from datetime import timedelta
now = timezone.now()
carte = CarteCashless.objects.get(tag_id='${DEMO_TAGID_CLIENT2}')
tx = Transaction.objects.filter(
    action=Transaction.SALE,
    card=carte,
...
if tx:
    ...
    print('NO_TX_FOUND')
`);
      console.log('DB Transaction CLIENT2:', result);
